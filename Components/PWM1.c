...
/**
 * @brief  PWM初始化   电机1初始化   gpio PA1推挽输出    对应TIM2_CH2输出比较通道  初始化通道2 ccr compare2
 * @param
 * @retval
...
void PWM1_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_1;//对应TIM2_CH3输出比较通道3     GPIO_Pin_0;对应是可外部输入TIM2_CH1_ETR    GPIO_Pin_1  通道1对应定时器输出比较通道2 TIM_OC2Init
    ...
    TIM_TimeBaseStructure.TIM_Prescaler         = 36 - 1;            // 预分频    psc    拦截7100   给1个脉冲给计数器             720-1
    ...
    TIM_OC2Init(TIM2, &TIM_OCInitStructure);//直流电机用通道x输出  
 ...
 * @note
 */
void PWM1_SetCompare(uint16_t CompareValue)
{
    TIM_SetCompare2(TIM2, CompareValue);
}
 ...
 * @note   补充
 */
// void PWM1_SetPeriod(uint16_t arr, uint16_t psc)
// {
...
//     TIM_SetCompare2(TIM2, 0); // 防止切换时误输出
// }
